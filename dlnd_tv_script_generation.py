...
def get_batches(int_text, batch_size, seq_length):
    ...
    inputs = np.array(int_text[0:character_used])

    targets = np.zeros_like(inputs)
    targets[:-1] = int_text[1:character_used]    
    if len(int_text) == character_used:
        targets[-1] = inputs[0]
    else:
        targets[-1] = int_text[character_used]
    ...
    for n in range(0, inputs.shape[1], seq_length):
        one_input = np.array(inputs[:,n:n+seq_length])
        one_target = np.array(targets[:, n:n+seq_length])
        one_batch_output = np.stack((one_input, one_target))
...
def pick_word(probabilities, int_to_vocab):
    ...
    pred_word = int_to_vocab[word_id]
    return pred_word
